...
def is_up_to_date() -> bool:
    ...
    try:
        ...
        local = subprocess.check_output(["git", "rev-parse", "@"], stderr=subprocess.DEVNULL).strip()
        remote = subprocess.check_output(["git", "rev-parse", "@{u}"], stderr=subprocess.DEVNULL).strip()
        base = subprocess.check_output(["git", "merge-base", "@", "@{u}"], stderr=subprocess.DEVNULL).strip()
...
def warn_if_outdated(obj):
    print("c est le decorateur")
    @functools.wraps(obj)
    def wrapper(*args, **kwargs):
        print("On verifie si le projet est à jour")
        global _warned_once
